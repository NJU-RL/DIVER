...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            hidden_states_lst.append(hidden_states.mean(dim=1))
    ...
    def _compute_group_constrastive_loss(self, old_hidden_states: torch.Tensor, hidden_states: torch.Tensor, temperature=0.07):
        """
        ...
        old_hidden_states_norm = torch.nn.functional.normalize(old_hidden_states, dim=2)
        hidden_states_norm = torch.nn.functional.normalize(hidden_states, dim=2)
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    hidden_states = hidden_states.mean(dim=1).unsqueeze(dim=1)
                    ...
                    loss = policy_loss / self.gradient_accumulation + 0.1 * constrastive_loss
                    loss.backward()
