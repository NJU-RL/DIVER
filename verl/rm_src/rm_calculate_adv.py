...
def compute_reinforce_plus_plus_outcome_advantage(token_level_rewards: torch.Tensor, eos_mask: torch.Tensor,
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    # print("*****token_level_rewards: ",token_level_rewards)
    # print("*****token_level_rewards.shape: ",token_level_rewards.shape)
    # print("rule_score:", token_level_rewards.sum(dim=-1))
    with torch.no_grad():
        ...
        #     returns *= shaping_reward
        # print("adv_pre",verl_F.masked_whiten(returns, eos_mask))
        # todo: 
        if shaping_reward is not None:
            advantages = verl_F.masked_whiten(returns+shaping_coef*shaping_reward, eos_mask)
        else:  
